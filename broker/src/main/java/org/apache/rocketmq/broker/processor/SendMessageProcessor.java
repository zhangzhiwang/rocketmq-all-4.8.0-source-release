...
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx,
        ...
        try {
            //这里调用的方式，为啥有一个 get()方法（FutureTask   玩过 刷1 没有刷2）
    ...
    private CompletableFuture<RemotingCommand> asyncSendMessage(ChannelHandlerContext ctx, RemotingCommand request,
        ...
        //todo 在发送前的处理，标识RPC的SeqNumber,检查队列的读写权限、自动创建Topic等等
        ...
        //todo  如果没有指定QueueID,系统随机指定一个
        if (queueIdInt < 0) {
            queueIdInt = randomQueueId(topicConfig.getWriteQueueNums());
        }
        //todo 构造Broker内部使用的Message（包装一层）
        MessageExtBrokerInner msgInner = new MessageExtBrokerInner();
        msgInner.setTopic(requestHeader.getTopic()); //主题
        msgInner.setQueueId(queueIdInt);          //queueId
        //todo 死信消息的处理逻辑--如果是消费重试次数达到上线，就会进入死信队列
        ...
        if (transFlag != null && Boolean.parseBoolean(transFlag)) {
            ...
            //todo 分布式事务 消息的Prepare消息
            putMessageResult = this.brokerController.getTransactionalMessageService().asyncPrepareMessage(msgInner);
        } else {
            //todo 处理普通消息，和事务消息的Commit/Rollback消息
